...
export function applyAmountToTotal(payment: Payment, amount: number, occurrenceKey: string): Payment {
  ...
  if (alreadyPaid) {
    ...
    return { ...payment, total: Math.max(0, (payment.total ?? 0) + amountToAddBack), paidAmounts };
  }
  const amountToApply = Math.min(amount, payment.total ?? 0);
  paidAmounts[occurrenceKey] = amountToApply;
  return { ...payment, total: Math.max(0, (payment.total ?? 0) - amountToApply), paidAmounts };
}

export function getSnowballAmount(payments: Payment[]): number {
  return payments.find(p => p.id === "SNOWBALL")?.amount ?? 0;
}

export function computeUpdatedPayment(original: Payment, virtual: Payment): Payment {
  const occurrenceDate = original.interval === "YEARLY"
    ? new Date(virtual.dueDate.toDate().getFullYear(), original.dueDate.toDate().getMonth(), original.dueDate.toDate().getDate())
  ...
  if (original.type === "DEBT" || original.type === "FUND") {
    const withTotal = applyAmountToTotal(original, virtual.amount, occurrenceKey);
