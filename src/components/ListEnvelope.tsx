...
export default function ListEnvelope({ envelope, onClick}: {envelope: Envelope, onClick: () => void}) {
    return (
        <div className={`
            ...
            w-screen max-w-[40rem] h-[2rem] grid grid-cols-5 divide-x-2 divide-my-black-dark
            border-2 border-my-black-dark cursor-pointer`}
            onClick={onClick}>

                <div className="col-span-3 flex justify-start items-center ml-8 gap-10 relative">
                    <div className="w-fit h-[85%] flex justify-center items-center bg-white rounded-lg">
                        <BiEnvelope className="w-[2rem] h-[2rem] text-my-black-dark" />
                    </div>
                    <p className="text-sm absolute left-1/2 -translate-x-1/2">{envelope.name}</p>
                </div>
